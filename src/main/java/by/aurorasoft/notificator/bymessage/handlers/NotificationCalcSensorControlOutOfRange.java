...
public class NotificationCalcSensorControlOutOfRange extends NotificationCalcSensor {

    double min;
    double max;

    public NotificationCalcSensorControlOutOfRange(Meta meta, boolean isContinuable, Sensor sensor, double min, double max) {
    ...
    public boolean isActive(MessageEntity prev, MessageEntity current, SensorBunchCalculator calculator) {
        double v = calculator.calculate(sensor.getId(), current);
        return v > max || v < min;
    }
}
